...
public class WsColegio {
	...
	//Muestra todos los datos de nuestra tabla de Estudiantes
	@GET
	@Path("/getEstudiantes")
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	...
	//Sirve para pedir los datos de un estudiante de nuestra tabla Estudiantes
	@POST
	@Path("/getEstudiantesById")
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String getEstudiantesById(String idEstudiante) {
		Gson gson = new Gson();
		List<Estudiante> estudiantes = new ArrayList<Estudiante>();
		...
		try {
			Connection conn = BaseDatos.getConnection();
			ps = conn.prepareStatement("SELECT  * from Estudiantes WHERE idEstudiante = ?");
			ps.setInt(1, Integer.valueOf(idEstudiante));
			rs = ps.executeQuery();
